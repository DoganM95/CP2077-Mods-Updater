...
exports.runOnce = async () => {
    ...
    for (const mod of mods) {
        await exports.processTool(mod.id, mod.repo, mod.relpath);
...
// Replaces cp -a command (copy files recursively)
...
exports.processTool = async (id, repo, relpath) => {
    exports.log("-----");
    const installPath = relpath.startsWith("/") ? relpath : path.join(process.env.GAME_DIR, relpath);
    ...
    try {
        ...
        // Determine if zip created a single top-level dir
