...
exports.copyRecursive = (extractedFolder, dest) => {
    ...
    for (const entry of entries) {
        if (entry.name === "__MACOSX" || (entry.name.startsWith(".") && !entry.name.endsWith(".cet"))) continue; // Skip Mac junk and hidden files (except .cet which is legit)
    ...
    try {
        ...
        const matching = (json.assets || [])
            .filter((a) => {
                ...
                const filename = a.name || path.basename(a.browser_download_url); // Extract and test filename only (not full URL)
        ...
        if (repo === "jac3km4/redscript") {
            ...
            return { tag: null, zipUrls: [] }; // Prevent fallback to source
        }
        exports.log(`No asset matched regex for ${repo}, trying zipball as last resort...`);
        return { tag, zipUrls: [json.zipball_url] }; // Fallback as last resort
    } catch (err) {
